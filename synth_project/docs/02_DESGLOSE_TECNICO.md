...
## DigitalSynth v1.0 — Ingeniería, Riesgos y Complicaciones

> **v2.0 — Arquitectura 100% Digital (réplica funcional Elektron Digitone)**
> Sin cadena analógica de señal. El filtro SVF y VCA por voz se implementan en DSP sobre STM32H743.
...
### 4.1 Esquema de Fuentes (simplificado — sin rail negativo)
...
USB-C (5V / 1.5A)
    ...
    │                            MCP23017, PCA9685, encoders, botones
    │
    └── LT3042 configurado a 3.3V ──► +3.3V_ANA @ 200mA
            │                           │
         (noise < 0.8µV RMS)   PCM5122 AVDD + CPVDD (sección analógica del DAC)

Sin LT3094. Sin LT1054. Sin rail -5V.

DAC8565 AVDD: conectado a USB +5V via ferrita bead (CV out no es crítico en ruido).
DAC8565 IOVDD: conectado a +3.3V_DIG.
```
...
2. **LT3042 (PSRR > 76dB @ 100kHz)** — filtra cualquier ruido remanente del USB hacia PCM5122
3. **Sin LT3094 / LT1054** — al eliminar la cadena analógica se eliminan esas dos fuentes de ruido
...
□ Medir resistencia VBUS a GND: debe ser > 1kΩ (si es 0Ω = cortocircuito)
□ Medir resistencia +3.3V_DIG a GND: debe ser > 500Ω
□ Medir resistencia +3.3V_ANA a GND: debe ser > 500Ω
□ Verificar orientación de todos los electrolíticos con lupas
□ Verificar orientación del STM32H7 (punto de pin 1)
□ Verificar orientación del PCM5122 y DAC8565
...
| Ruido de fuente en audio PCM5122 | Baja | Medio | LT3042 @ 3.3V_ANA: PSRR > 76dB. Sin rail negativo → sin LT1054 como fuente de ruido |
...
| Motor FM produce artefactos (aliasing, DC offset) | Media | Medio | Filtro DC-blocking por voz, oversampling x2 si hay aliasing audible |
| Algoritmo FM no suena como Digitone | Media | Bajo | Digitone usa FM 4-op estándar: comparar en oscilloscope vs Digitone usando mismo preset |
