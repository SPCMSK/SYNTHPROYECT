...
## DigitalSynth v1.0 — Bill of Materials Completo

> **Versión:** 2.0 — Febrero 2026 (pivote arquitectónico: 100% digital, sin cadena analógica)
> **Concepto:** Réplica funcional de Elektron Digitone — 4 voces FM, filtro digital por voz, secuenciador de 8 tracks
> **Moneda:** USD (precio unitario, sin envío ni impuestos)
> **Proveedores referenciados:** Mouser, DigiKey, LCSC, AliExpress (prototipo)

---

## RESUMEN ECONÓMICO

> ⚠️ **v2.0 — Arquitectura 100% Digital:** Se eliminaron AS3320 (VCF), THAT2180 (VCA), OPA2134, NE5532,
> relay bypass, rail negativo (LT3094 + LT1054) y todo el circuito analógico de señal.
> El filtro y VCA por voz se implementan en firmware (DSP digital en STM32H743).
> El DAC8565 se reutiliza como salida de 4× CV para equipos externos (Eurorack).

| Categoría | Costo Estimado |
|---|---|
| Semiconductores y ICs activos | $45–60 |
| Componentes pasivos | $8–15 |
| Conectores y mecánicos | $20–35 |
| PCB fabricación (5 piezas, JLCPCB) | $12–20 |
| Módulos y subcomponentes | $10–20 |
| Herramientas y consumibles (una vez) | $50–100 |
| **TOTAL ESTIMADO (1 unidad)** | **$145–250 USD** |

> **Ahorro vs v1.0 HybridSynth:** ~$50–65 en ICs analógicos eliminados. PCB más pequeño (sin sección analógica).
...
| Qty | Componente | Descripción | Referencia | Proveedor | Precio Unit. | Notas |
|---|---|---|---|---|---|---|
| 1 | **PCM5122PW** | DAC audio I2S 32bit/384kHz, 112dB, TSSOP28 | PCM5122PW | Mouser | $3.20 | Salida estéreo de audio FM — reemplaza PCM5242 (mismo footprint, mismos registros I2C, sin PLL externo) |
| 1 | **DAC8565IAPWR** | DAC 16bit, 4 canales, referencia interna 2.5V, SPI, TSSOP16 | DAC8565IAPWR | DigiKey | $7.50 | **CV output externo:** 4× jacks TRS 3.5mm para Eurorack. VOUTB→pitch, VOUTA→gate, etc. Reemplaza DAC8564 (misma familia, LDAC nivel activo invertido) |
...
## SECCIÓN 3 — FILTRO DIGITAL POR VOZ (DSP firmware)

> **Implementado 100% en firmware STM32H743 — sin componentes hardware adicionales.**
> Cada una de las 4 voces tiene su propio filtro SVF (State Variable Filter) digital:
> — Lowpass / Highpass / Bandpass seleccionable por parámetro
> — Cutoff: 20Hz–20kHz con tabla exponencial (1024 coeficientes biquad precalculados)
> — Resonancia: 0 a auto-oscilación controlada
> — Carga CPU estimada: 4 voces × 2 biquad = ~0.3% del STM32H7 @ 480MHz

| Qty | Componente | Descripción | Referencia | Proveedor | Precio Unit. | Notas |
|---|---|---|---|---|---|---|
| — | *(sin hardware adicional)* | Filtro SVF implementado en firmware | — | — | $0 | Ver `firmware/dsp/` — coeficientes biquad por voz, actualización en ISR DMA SAI1 |

---

## SECCIÓN 4 — VCA DIGITAL + BUFFER DE SALIDA

> **VCA por voz implementado en firmware** (multiplicación de muestra por envolvente — 0 hardware extra).
> Solo se necesita un buffer de salida pasivo en la salida del PCM5122.

| Qty | Componente | Descripción | Referencia | Proveedor | Precio Unit. | Notas |
|---|---|---|---|---|---|---|
| 2 | Resistor 100Ω | 1%, 0402 | — | LCSC | $0.02 c/u | Resistor de aislamiento por canal (L y R) entre PCM5122 OUTPUT y jacks |
| 2 | Condensador 2.2nF | C0G, 0402 | — | LCSC | $0.05 c/u | Filtro RC de salida (con 100Ω → fc = 720kHz) |
| 2 | Jack 6.35mm (1/4") TRS | Audio output estéreo, panel | — | Mouser | $1.50 c/u | Salida L + R |
| 2 | Jack RCA, panel | Salida RCA estéreo | — | LCSC | $0.40 c/u | Salida L + R alternativa |
...
| 1 | **LT3042EMSE** | LDO ultra bajo ruido +3.3V_ANA, 200mA, MSOP10 | LT3042EMSE | Mouser | $5.50 | Rail analógico del PCM5122 (AVDD + CPVDD). Configurado a 3.3V con resistor SET. |
| ~~1~~ | ~~**LT3094EMSE**~~ | ~~LDO -5V_ANA~~ | — | — | **ELIMINADO** | Sin cadena analógica de audio → sin rail negativo |
| ~~1~~ | ~~**LT1054ISW**~~ | ~~Charge pump -5.5V~~ | — | — | **ELIMINADO** | Sin rail negativo → charge pump no necesaria |
...
> **Nota:** Alfa Rivas (AS3320) eliminada del listado — diseño 100% digital, sin ICs de synth analógico.
