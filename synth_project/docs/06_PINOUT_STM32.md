...
## DigitalSynth v1.0 — Referencia para STM32CubeMX
...
6. [SAI1 — Audio I2S → PCM5122](#sai1--audio-i2s--pcm5122)
7. [SPI1 — DAC de CV → DAC8565](#spi1--dac-de-cv--dac8565)
...
## 6. SAI1 — AUDIO I2S → PCM5122

El STM32 actúa como **master** — genera todos los clocks. El PCM5122 es esclavo.
Se usa la interfaz SAI (Serial Audio Interface) que en el STM32H7 reemplaza al I2S clásico.

| Pin STM32 | Función CubeMX | Net KiCad | Va a PCM5122 pin | Descripción |
|---|---|---|---|---|
...
## 7. SPI1 — DAC DE CV → DAC8565

El DAC8565 genera 4 voltajes analógicos (0–5V, referencia interna 2.5V) para salidas CV
hacia equipos externos (Eurorack, sintetizadores modulares). No controla nada interno —
el filtro y VCA son digitales en firmware.

| Pin STM32 | Función CubeMX | Net KiCad | Va a DAC8565 pin | Descripción |
|---|---|---|---|---|
| PA5 | SPI1_SCK | DAC_SCK | Pin 10 (SCLK) | Clock SPI — hasta 50 MHz |
| PA7 | SPI1_MOSI | DAC_MOSI | Pin 11 (DIN) | Datos hacia el DAC (MSB first, 24 bits por escritura) |
| PB6 | GPIO_OUT | DAC_CSn | Pin 9 (~SYNC) | Chip Select activo LOW — gestión manual en firmware |
| PB7 | GPIO_OUT | DAC_LDAC | Pin 16 (LDAC) | Update latch — **flanco de SUBIDA** actualiza los 4 canales simultáneamente (DAC8565 ≠ DAC8564) |
| PC4 | GPIO_OUT | DAC_RST | Pin 13 (~RST) | Reset activo LOW, pull-up 10kΩ a +3.3V |

**Asignación de los 4 canales DAC8565 — CV out externo:**

| Canal | Net | Jack | Voltaje | Para qué |
|---|---|---|---|---|
| DAC_A | CV_OUT_A | TRS 3.5mm #1 | 0–5V | CV out libre para gear externo (pitch, mod, etc.) |
| DAC_B | CV_OUT_B | TRS 3.5mm #2 | 0–5V | CV out libre para gear externo |
| DAC_C | CV_OUT_C | TRS 3.5mm #3 | 0–5V | CV out libre para gear externo |
| DAC_D | CV_OUT_D | TRS 3.5mm #4 | 0–5V | CV out libre para gear externo |

> El firmware puede mapear cualquier parámetro interno (LFO, envolvente, pitch de voz) a cualquier canal.
> Asignación sugerida: A=pitch voz 1, B=filtro cutoff voz 1, C=libre, D=gate/trigger.

**LDAC firmware — DAC8565 (flanco de subida, NO pulso LOW):**
```c
// Mantener LDAC en LOW mientras se escriben los canales,
// luego HIGH para actualizar los 4 canales simultáneamente
HAL_GPIO_WritePin(DAC_LDAC_GPIO_Port, DAC_LDAC_Pin, GPIO_PIN_RESET);
DAC8565_WriteChannel(0, value_a);
DAC8565_WriteChannel(1, value_b);
DAC8565_WriteChannel(2, value_c);
DAC8565_WriteChannel(3, value_d);
HAL_GPIO_WritePin(DAC_LDAC_GPIO_Port, DAC_LDAC_Pin, GPIO_PIN_SET); // ← trigger
```

**En CubeMX:**
- `SPI1 → Mode: Transmit Only Master` (DAC8565 no tiene SDO — solo DIN)
- Data Size: `8 bits` (se envían 3 bytes por canal con CS manual)
- Clock Polarity (CPOL): `Low`
- Clock Phase (CPHA): `2 Edge` (CPOL=0 CPHA=1 = Modo SPI 1)
- NSS: `Software` (CS controlado manualmente via PB6)
- Prescaler: Baudrate ≤ 50 MHz (SPI1 desde PCLK2 ≤ 240 MHz → Prescaler /8 = 30 MHz)
...
| PCM5122 | 0x4C (ADR1=L, ADR0=L) | Configuración del DAC de audio (volumen digital, formato, filtros) |
...
| PC3 | GPIO_Output | PCM5122_XSMT | Soft Mute del PCM5122 (XSMT pin 25) — HIGH = operación normal, LOW = mute. Pull-up 10 kΩ a +3V3_DIG |
...
| PC2 | ADC3_INP0 — CV_IN |
| PC3 | GPIO_OUT — PCM5122_XSMT |
| PC4 | GPIO_OUT — DAC8565_RST |
| PC5 | **LIBRE** |
| PC6 | TIM3_CH1 — ENC2_A |
| PC7 | TIM3_CH2 — ENC2_B |
...
```
Pinout → SPI1
  Mode: Transmit Only Master  (DAC8565 no tiene SDO)
...
| PC3 | GPIO_Output (init HIGH) | PCM5122_XSMT |
